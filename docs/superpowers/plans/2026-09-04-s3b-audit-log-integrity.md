...
Expected: mutant A reds `test_a_file_landing_with_the_wrong_group_refuses`. ~~Mutant B is the subtle one — `open(dst,'w')` is only reached when the file does not already exist, so it reds via the umask path (`0644` instead of `0660`) in `test_apply_creates_one_empty_log_per_registered_client_at_0660`.~~ **CORRECTED, see the design doc's §6.5 (two rounds of correction there, in full)**: `bootstrap_logs` calls `os.chmod(dst, LOG_FILE_MODE)` immediately after creating the file, which defeats the umask regardless of how the file was created — the `0644` prediction above is false and this mutant is inert against that test. What actually reds mutant B is the TOCTOU test (`test_a_toctou_race_between_the_exists_check_and_create_refuses_without_clobbering`): it forces `os.path.exists` to miss once for the target path while the file genuinely exists on disk, then asserts the call refuses AND the pre-existing bytes are unchanged — `open(dst,'w')` would silently truncate the winner's file instead of raising `FileExistsError`, killing that test. Report both sorted failing-set differences against the TOCTOU test, not the mode test. **If either mutant reds nothing, chase it** — an inert mutation is a coverage claim, not a pass. In particular, if mutant B is inert, the `assert` guards above did not match and the mutation never applied.
...
class TestRegisteredClientLogs(Base):
    ...
    def test_a_registered_client_without_a_log_is_refused(self):
        ...
        # The discriminating partner to the two-client test below: together they prove the
    ...
    def test_the_refusal_names_a_count_not_the_slugs(self):
        ...
        problems = PF.check(self.root, self.other_uid, self.gid, platform="linux")
        self.assertEqual(len(problems), 1)
        # CORRECTED 2026-09-04 during execution: this was assertIn("2", ...), which is
        # VACUOUS — the message interpolates LOG_DIR_MODE as %04o, i.e. "2750", so a "2"
        # is present however wrong the count is. Verified: a message built with
        # missing=99 still satisfies assertIn("2"). Match the whole phrase instead.
