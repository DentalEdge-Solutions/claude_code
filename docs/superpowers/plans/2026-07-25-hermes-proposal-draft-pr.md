...
- [ ] **Step 2: As the bot, a direct push to `main` is REJECTED server-side**
...
- [ ] **Step 3: As the bot, a non-base branch push + draft PR SUCCEEDS, then clean up**
...
Append the three results (env-scrub, main-rejected, branch+PR-ok, gh-vs-REST) to the SDD ledger. No commit (no code changed).
...
def render_candidate(proposal_text, proposal_path, run_id, now):
    ...
    return filename, fm + proposal_text if proposal_text.endswith("\n") else fm + proposal_text + "\n"
...
def main(argv=None):
    ...
    plan = build_plan(args.project, args.proposal, args.run_id, datetime.datetime.utcnow())
    if args.dry_run:
...
def _run_live(plan, project):
    ...
    tgt = plan["target"]; ws = tempfile.mkdtemp(prefix="proposal-pr-")
    try:
        url = f"https://x-access-token:{pat}@github.com/{tgt['repo']}"
        subprocess.run(["git", "clone", "--depth", "1", url, ws], check=True,
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # token not echoed
        # install the pre-push hook, pinned to the base branch
        hook_src = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pre-push-refuse-base.sh")
        hook_dst = os.path.join(ws, ".git", "hooks", "pre-push")
        shutil.copyfile(hook_src, hook_dst); os.chmod(hook_dst, 0o755)
        _git(ws, "config", "core.hooksPath", ".git/hooks")
        _git(ws, "config", "user.email", "bot@dentaledge.local")
        _git(ws, "config", "user.name", "hermes-bot")
        _git(ws, "checkout", "-b", plan["branch"])
        dest = os.path.join(ws, plan["dest"])
        os.makedirs(os.path.dirname(dest), exist_ok=True)
        with open(dest, "w", encoding="utf-8") as f:
            f.write(plan["content"])
        _git(ws, "add", plan["dest"])
        _git(ws, "commit", "-m", f"proposal: {plan['title']} (Hermes AIOS, machine-generated)")
        env = {**os.environ, "PREPUSH_BASE": tgt["base"]}
        subprocess.run(["git", "-C", ws, "push", "origin", plan["branch"]], check=True,
                       env=env, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        pr_url = _open_draft_pr(tgt, plan, pat)
        print(pr_url)
        return 0
    finally:
...
```bash
# (a) :ro mount byte-identical
docker compose exec -T hermes-agent sh -c 'cd /projects/claude_code && git status --porcelain | sort | sha256sum'   # == Step 1 hash
# (b) PAT not exposed in logs (fixed-string match; token never echoed)
docker compose exec -T hermes-agent sh -c '[ -n "$CLAUDE_CODE_PR_PAT" ] && grep -rlF "$CLAUDE_CODE_PR_PAT" /opt/data/logs 2>/dev/null && echo LEAK || echo "no token in logs"'
...
- **Spec coverage:** machine-account identity + admin-bypass ruleset (Prereqs + Task 1) ✓; empirical bot-run verification incl. env-scrub (Task 1) ✓; structured `pr_target` (Task 2 Step 1) ✓; candidate filename convention + frontmatter + provenance `author:hermes`/`hermes-generated`/`sources` (Task 2 render_candidate + tests) ✓; `:ro` mount never written / ephemeral clone / cleanup on success+failure (Task 3 `_run_live` finally) ✓; draft-only PR via gh|REST (Task 3) ✓; pre-push base refusal (Task 3 hook + tests) ✓; 90-day expiry + rotation doc (Prereq 3 + Task 4 README) ✓; PAT-not-exposed + `:ro` byte-identical (Task 4 Step 3) ✓.
- **Placeholder scan:** none — every code/command step is literal. `_run_live` is explicitly introduced in Task 3 (referenced by Task 2's `main` with a note); not a placeholder.
- **Type/name consistency:** `read_pr_target`, `resolve_proposal`, `render_candidate`, `build_plan`, `_run_live`, `_open_draft_pr`, the `{repo,base,path}` dict, `PROPOSALS_DIR`/`PR_REGISTRY` env, `CLAUDE_CODE_PR_PAT`, `PREPUSH_BASE`, and the `proposal/<ts>` branch name are used identically across tasks.
- **Ambiguity check:** the gh-vs-REST choice is resolved empirically in Task 1 Step 3 and both paths are implemented; the registry hand-parser is unit-tested for the exact structure it must read.
...
- Task 1 is a hard gate: if the bot can push to `main`, STOP.
- Never echo `$CLAUDE_CODE_PR_PAT`; keep it out of logs and out of any printed URL.
